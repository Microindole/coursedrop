...
当前已进入“文件链路联调”阶段，不再是单纯页面骨架阶段。下一步优先按下面顺序推进：

1. 本地库交互稳定：滚动、搜索、分页、详情抽屉、批量操作不能互相遮挡。
2. 本地库到分享草稿：选中文件后必须真正进入当前分享会话。
3. 公网分享创建：创建分享后拿到服务端分享链接。
4. multipart 上传：本地文件内容上传到中转服务器。
5. 浏览器下载：电脑浏览器能通过分享链接登录并下载。
6. 二维码/扫码：分享二维码展示、扫码登录、模拟器扫码调试。
7. 端到端加密：客户端加密、服务端只存密文和元数据、下载端本地解密。
8. App 内下载入库：下载到 CourseDrop 应用目录并写入本地索引。

详细拆解见 `docs/architecture/client-completion-plan.md`。当前不要把精力转到普通文件管理器或系统分享替代品上，先稳定下面这个闭环：

```text
本地库选文件 -> 分享草稿 -> 创建公网链接 -> 上传文件 -> 浏览器下载
```

这个闭环稳定后，再做二维码、App 内下载、端到端加密、账号安全和局域网直传。
...
- `account-security`：账号密码登录开关、改密码、设备指纹列表、绑定和解绑。
...
1. 端到端加密协议需要和鸿蒙客户端联调：默认算法、KDF 参数、密钥来源、解密和完整性校验。
2. Web 端已具备 `#key=` + WebCrypto AES-GCM 本地解密基础能力，后续需要和真实客户端生成的密文联调。
3. 当前限流是进程内实现，生产部署可替换为 Redis 或网关限流。
4. 当前迁移服务是轻量内部实现，生产部署可替换为 Flyway 或 Liquibase。
5. 测试已覆盖主流程、扫码 Cookie、加密元数据、下载策略、首页、账号安全和分享管理；后续可继续补 room/transfer 兼容接口和清理任务细节。

下一轮建议顺序：

1. 继续让鸿蒙客户端接入 server API：上传文件、撤回分享、续期、账号安全设置。
2. 在客户端实现端到端加密，再和服务端元数据校验联调。
3. 根据真实联调结果完善浏览器下载页和分享管理体验。
4. 部署到 `coursedrop.microindole.me`，用 GitHub Actions 的 main 分支自动发布持续联调。

当前没有鸿蒙真机时，扫码登录可用两种方式测试：

- 安卓手机扫码打开 `/m/login/{loginCode}`，输入鸿蒙模拟器设置页中的 `fingerprintId` 确认。
- 鸿蒙模拟器设置页直接输入网页登录码，调用确认接口模拟扫码结果。
